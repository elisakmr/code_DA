...
tictoc::tic()
  ...
  mapbiomas.basin.lsm = calculate_lsm(raster(mapbiomas[[y]]),level="class",metric = c("ca","pland","np"),directions =8)
...
tictoc::tic()
  ...
  if(nrow(nascente)>0){
    ...
    #computing landscape metrics at basin scale (MapBiomas)
    mapbiomas.basin.lsm = calculate_lsm(raster(mapbiomas_buf[[y]]),level="class",metric = c("ca","pland","np"),directions =8)
  ...
  save(mapbiomas.basin.df, file = file.path(WD.tmp, paste0(year, "_mapbiomas_lsm_buf",tampon, "_",i,".RData")))
