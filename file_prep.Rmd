...
## Setting up basin files TBRUN once only!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
for (i in 1:2031){
  ...
  terra::writeRaster(mapbiomas_mask_utm, file.path(WD.tmp, paste0("mapbiomas_", i, ".tif")), overwrite = TRUE)
...
for (i in 1:10){
  ...
  terra::writeRaster(modis_mask_utm, file.path(WD.tmp, paste0("modis_", i, ".tif")), overwrite = TRUE)
...
tictoc::tic()
  ...
  foreach(i=c(11:2031), .packages=c('sf')) %dopar%  { #
    ...
    st_write(crop.utm, file.path(WD.tmp, paste0("riossimple_",i,".shp")), delete_layer = TRUE)
...
tictoc::tic()
  ...
  foreach(i=c(1:2031), .packages=c('sf')) %dopar%  { #
    ...
    if(polynetwork[i,]!=0){
      ...
      st_write(crop.utm, file.path(WD.tmp, paste0("riospoly_",i,".shp")), delete_layer = TRUE)
...
tictoc::tic()
  ...
  foreach(i=c(1:2031), .packages=c('sf')) %dopar%  { #
    ...
    WD.tmp = list.files(path=dir_data, pattern = paste0("Basin_",i,"_"), full.names = TRUE)
      ...
      st_write(crop.utm, file.path(WD.tmp, paste0("nascente_",i,".shp")), delete_layer = TRUE)
...
tictoc::tic()
  ...
  foreach(i=c(1:2031), .packages=c('sf')) %dopar%  { #
    ...
    WD.tmp = list.files(path=dir_data, pattern = paste0("Basin_",i,"_"), full.names = TRUE)
      ...
      st_write(crop.utm, file.path(WD.tmp, paste0("reserv_",i,".shp")), delete_layer = TRUE)
